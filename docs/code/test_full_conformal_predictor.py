#!/usr/bin/env python3
"""
Полный тест conformal_predictor с обучением и созданием прогнозов.
"""
...
def test_full_conformal_predictor():
    """Полный тест conformal_predictor с обучением и созданием прогнозов."""
    print("🧪 ПОЛНЫЙ ТЕСТ CONFORMAL_PREDICTOR")
    print("=" * 50)
    ...
    try:
        ...
        # Обучаем предиктор
        print("\\n🎓 Обучение предиктора...")
        training_success = predictor.train_conformal_predictor()
        
        if not training_success:
            print("❌ Ошибка обучения предиктора")
            return
        
        print("✅ Предиктор обучен успешно")
        ...
        # Берем только первые 2 чемпионата для тестирования
        test_tournament_ids = tournament_ids[:2]
